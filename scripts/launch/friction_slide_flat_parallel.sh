...
# Non-Docker Parallel Friction Slide Flat Simulation Launch Script
# This script runs parallel friction slide flat simulations using a specific Python interpreter across multiple GPUs

# Set default values
PYTHON_PATH="/workspace/blender-3.4.0-linux-x64/3.4/python/bin/python3.10"
# OUTPUT_DIR="/net/acadia1a/data/sriram/vidgen/datasets/kubric_generated/friction_slide_flat_v2"
## Varying 3 ranges Low, High and Random - 15k videos
OUTPUT_DIR="/net/acadia1a/data/sriram/vidgen/datasets/kubric_generated/friction_slide_flat_v3"
NUM_WORKERS=12
NUM_VIDEOS=500
RESOLUTION="768x432"
FRAME_END=97
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --help)
      ...
      echo "  --num_videos N             Total number of videos to generate (default: 500)"
      ...
      echo ""
      echo "Note: This script will run parallel friction slide flat simulations using the specified Python interpreter."
      echo "      Each video will randomly vary friction coefficients on flat platform slides."
      echo ""
      echo "Example:"
...
echo "================================================"
echo "Parallel Friction Slide Flat Simulation Launch (Non-Docker)"
echo "================================================"
...
echo "Friction variation: Random coefficients on flat platform"
...
echo "  Initial velocity: 2.0 m/s (positive y direction)"
echo "================================================"
...
echo "================================================"
echo "Starting parallel friction slide flat simulation..."
echo "================================================"

# Run the Python script
"$PYTHON_PATH" src/run_parallel_friction_slide_flat.py \
...
if [ $EXIT_CODE -eq 0 ]; then
    echo "✅ Parallel friction slide flat simulation completed successfully!"
    ...
    if [ -d "$OUTPUT_DIR" ]; then
        ...
        find "$OUTPUT_DIR" -name "metadata.json" | head -3 | while read metadata_file; do
            ...
            if command -v jq >/dev/null 2>&1; then
                echo "  Brick name: $(jq -r '.brick_name // "N/A"' "$metadata_file")"
                echo "  Platform friction: $(jq -r '.platform_friction // "N/A"' "$metadata_file")"
                echo "  Brick friction: $(jq -r '.object_data.friction[1] // "N/A"' "$metadata_file")"
                echo "  Initial velocity: $(jq -r '.velocity_profile.velocity_magnitude // "N/A"' "$metadata_file") m/s"
                echo "  Velocity direction: $(jq -r '.velocity_profile.velocity_vector // "N/A"' "$metadata_file")"
                echo "  Applied velocities: $(jq -r '.applied_velocities_simulator | length // 0' "$metadata_file") velocity applications"
            else
    ...
    echo "❌ Parallel friction slide flat simulation failed with exit code: $EXIT_CODE"
fi
echo "================================================"
