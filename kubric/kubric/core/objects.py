...
class PhysicalObject(Object3D):
  ...
  Attributes:
    ...
    friction (float): Friction coefficient used for physics simulation of this object
                      (between 0 and 1).
    rolling_friction (float): Rolling friction coefficient used for physics simulation of this object
                              (between 0 and 1).
  ...
  rolling_friction = tl.Float(0.01)
  spinning_friction = tl.Float(0.01)
  ...
  @tl.validate("rolling_friction")
  def _valid_rolling_friction(self, proposal):
    rolling_friction = proposal["value"]
    if rolling_friction < 0:
      raise tl.TraitError(f"rolling_friction cannot be negative ({rolling_friction})")
    if rolling_friction > 1.0:
      raise tl.TraitError(f"rolling_friction cannot be larger than 1.0 ({rolling_friction})")
    return rolling_friction

  @tl.validate("spinning_friction")
  def _valid_spinning_friction(self, proposal):
    spinning_friction = proposal["value"]
    if spinning_friction < 0:
      raise tl.TraitError(f"spinning_friction cannot be negative ({spinning_friction})")
    if spinning_friction > 1.0:
      raise tl.TraitError(f"spinning_friction cannot be larger than 1.0 ({spinning_friction})")
...
class FileBasedObject(PhysicalObject):
  ...
  # URDF origin offset for the object
  urdf_origin_offset = tl.Tuple(tl.Float(), tl.Float(), tl.Float(), allow_none=True)

  # TODO: trigger error when changing filenames or asset-id after the fact



class SoftBody(FileBasedObject):
  """A soft body object that can deform during physics simulation.
  
  Soft bodies require mesh files in specific formats (.vtk for PyBullet) and have
  additional parameters for controlling deformation behavior.
  
  Attributes:
    spring_elastic_stiffness (float): Stiffness of springs between vertices [0,∞].
    spring_damping_stiffness (float): Damping of spring oscillations [0,∞].
    spring_bending_stiffness (float): Resistance to bending deformation [0,∞].
    collision_margin (float): Margin for collision detection.
    use_mass_spring (bool): Whether to use mass-spring model.
    use_bending_constraints (bool): Whether to add bending constraints.
    use_neo_hookean (bool): Whether to use Neo-Hookean material model.
    neo_hookean_mu (float): Neo-Hookean material parameter mu.
    neo_hookean_lambda (float): Neo-Hookean material parameter lambda.
    neo_hookean_damping (float): Neo-Hookean damping parameter.
    cluster_count (int): Number of clusters for collision optimization.
    self_collision (bool): Whether to enable self-collision detection.
  """
  
  # Soft body specific parameters - REMOVED max=1.0 constraints
  spring_elastic_stiffness = tl.Float(0.5, min=0.0)  # Removed max constraint
  spring_damping_stiffness = tl.Float(0.5, min=0.0)  # Removed max constraint
  spring_bending_stiffness = tl.Float(0.5, min=0.0)  # Removed max constraint
  
  # Material model selection
  use_mass_spring = tl.Bool(True)
  use_bending_constraints = tl.Bool(False)
  use_neo_hookean = tl.Bool(False)
  
  # Neo-Hookean material parameters (when use_neo_hookean=True)
  neo_hookean_mu = tl.Float(100.0, min=0.0)
  neo_hookean_lambda = tl.Float(100.0, min=0.0)
  neo_hookean_damping = tl.Float(0.01, min=0.0)
  
  # Vertex Positions attribute for storing the vertex positions over time
  vertex_positions = tl.List(ktl.Vector3D())

  # Tet to Tri vertex mapping
  tri_to_tet_mapping_filename = tl.Unicode(allow_none=True)
  # Tri to Tet mapping
  tri_to_tet_mapping = tl.List(tl.List(tl.Int()), allow_none=True)
  
  # FIXED: Updated validation to allow higher stiffness values
  @tl.validate("spring_elastic_stiffness", "spring_damping_stiffness", "spring_bending_stiffness")
  def _valid_stiffness(self, proposal):
    value = proposal["value"]
    if value < 0.0:
      raise tl.TraitError(f"Stiffness values cannot be negative, got {value}")
    # Removed upper bound check - allow high stiffness values for stability
    return value
  
  @tl.validate("tri_to_tet_mapping_filename")
  def _valid_tri_to_tet_mapping_filename(self, proposal):
    value = proposal["value"]
    if value is not None:
      import pathlib
      path = pathlib.Path(value)
      if not path.exists():
        logger.warning(f"Tri-to-tet mapping file does not exist: {value}")
    return value
