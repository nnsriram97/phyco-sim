...
class ParallelJengaForceGenerator:
    ...
    def run_single_video(self, video_id: str, gpu_id: int, composition_style: str) -> bool:
        """Run a single Jenga force video generation using subprocess with specific GPU."""
        try:
            logger.info(f"Generating Jenga force video {video_id} on GPU {gpu_id} with composition style: {composition_style}")
            
            # Build command arguments
            cmd = [
                sys.executable,
                "src/run_jenga_force.py",
                f"--video_id={video_id}",
                ...
                f"--min_layers=12",
                f"--max_layers=20",
                f"--missing_block_probability=0.40",
                f"--force_layer_bias=0.8",
                f"--velocity_magnitude=1.0",
                f"--tower_stability_attempts=10",
            ...
            try:
                ...
                if process.returncode == 0:
                    logger.info(f"Successfully generated Jenga force video {video_id} on GPU {gpu_id}")
                    with self.lock:
                        self.generated_videos += 1
                    return True
                else:
                    logger.error(f"Failed to generate Jenga force video {video_id} on GPU {gpu_id}")
                ...
                process.kill()
                logger.error(f"Jenga force video {video_id} generation timed out on GPU {gpu_id}")
                with self.lock:
                    self.failed_videos += 1
                return False
            finally:
            ...
            logger.error(f"Exception while generating Jenga force video {video_id} on GPU {gpu_id}: {e}")
            with self.lock:
                self.failed_videos += 1
            return False
    
    def run_parallel_generation(self, num_workers: int, num_videos: int):
        """Run parallel Jenga force video generation using subprocesses with GPU assignment."""
        logger.info(f"Starting parallel Jenga force generation: {num_workers} workers, {num_videos} videos")
        ...
        # Only vary composition styles for Jenga force simulations
        composition_styles = [
            'center',           # Tower in center (original behavior)
            'left_third',       # Tower on left third of frame  
            'right_third',      # Tower on right third of frame
            'upper_left',       # Tower in upper left area
            'upper_right',      # Tower in upper right area
            'lower_center',     # Tower in lower center (more ground visible)
            'lower_left',       # Tower in lower left area
            'lower_right',      # Tower in lower right area
        ]        
        ...
        try:
            with ThreadPoolExecutor(max_workers=num_workers) as executor:
                ...
                for i, video_id in enumerate(video_id_list):
                    ...
                    future = executor.submit(self.run_single_video, video_id, gpu_id, composition_style_list[i])
                ...
                for future in as_completed(future_to_video):
                    ...
                    try:
                        ...
                        logger.info(f"Jenga force video {video_id} completed on GPU {gpu_id}: {status}")
                    except Exception as e:
                        logger.error(f"Jenga force video {video_id} failed on GPU {gpu_id} with exception: {e}")
                        with self.lock:
                            self.failed_videos += 1
                    ...
                    logger.info(f"Progress: {completed_count}/{num_videos} Jenga force videos completed "
                               f"({self.generated_videos} success, {self.failed_videos} failed), "
                               f"Rate: {rate:.2f} videos/sec, ETA: {eta/60:.1f} min")
        finally:
            self.cleanup_processes()
        
        logger.info(f"Parallel Jenga force generation completed: {self.generated_videos} successful, "
                   f"{self.failed_videos} failed")

def create_parser():
    """Create argument parser for parallel Jenga force video generation."""
    parser = kb.ArgumentParser()
    
    # Arguments from run_jenga_force.py
    ...
    parser.add_argument("--num_videos", type=int, default=100,
        help="Total number of Jenga force videos to generate"
    )
...
if __name__ == "__main__":
    ...
    logger.info(f"Generating {args.num_videos} Jenga force videos using {args.num_workers} workers")
    
    # Run parallel generation
    generator = ParallelJengaForceGenerator(args)
    ...
    try:
        ...
        logger.info("All Jenga force videos generated successfully!")
