...
class ParallelSoftBodyDropGenerator:
    ...
    def run_single_video(self, video_id: str, gpu_id: int, ball_neo_hookean_mu: float, ball_neo_hookean_damping: float, composition_style: str) -> bool:
        """Run a single soft body drop video generation using subprocess with specific GPU."""
        try:
            logger.info(f"Generating soft body drop video {video_id} on GPU {gpu_id}")
            
            # Build command arguments
            cmd = [
                sys.executable,
                "src/run_ball_drop_soft_v4.py",
                f"--video_id={video_id}",
                ...
                f"--ball_restitution=0.05",
                f"--platform_restitution=0.05",
                f"--platform_friction=1.0",
                f"--ball_friction=1.0",
                f"--ball_neo_hookean_mu={ball_neo_hookean_mu}",
                f"--ball_neo_hookean_damping={ball_neo_hookean_damping}",
            ...
            try:
                ...
                if process.returncode == 0:
                    logger.info(f"Successfully generated soft body drop video {video_id} on GPU {gpu_id}")
                    with self.lock:
                        self.generated_videos += 1
                    return True
                else:
                    logger.error(f"Failed to generate soft body drop video {video_id} on GPU {gpu_id}")
                ...
                process.kill()
                logger.error(f"Soft body drop video {video_id} generation timed out on GPU {gpu_id}")
                with self.lock:
                    self.failed_videos += 1
                return False
            finally:
            ...
            logger.error(f"Exception while generating soft body drop video {video_id} on GPU {gpu_id}: {e}")
            with self.lock:
                self.failed_videos += 1
            return False
    
    def run_parallel_generation(self, num_workers: int, num_videos: int):
        """Run parallel soft body drop video generation using subprocesses with GPU assignment."""
        logger.info(f"Starting parallel soft body drop generation: {num_workers} workers, {num_videos} videos")
        ...
        # Sample damping stiffness coefficients for soft body deformation
        ...
        neo_hookean_mu_list = []
        neo_hookean_damping_list = []
        
        for i, video_id in enumerate(video_id_list):
            if deformability_range[i] == 0:
                # High deformability (low damping stiffness)
                neo_hookean_mu_list.append(np.random.uniform(60, 100))
                neo_hookean_damping_list.append(np.random.uniform(0.01, 0.1))
            elif deformability_range[i] == 1:
                # Low deformability (high damping stiffness)
                neo_hookean_mu_list.append(np.random.uniform(500, 600))
                neo_hookean_damping_list.append(np.random.uniform(0.9, 1.0))
            else:
                # Mixed deformability values
                neo_hookean_mu_list.append(np.random.uniform(60, 600))
                neo_hookean_damping_list.append(np.random.uniform(0.01, 1.0))
        composition_styles = [
        ...
        try:
            with ThreadPoolExecutor(max_workers=num_workers) as executor:
                ...
                for i, video_id in enumerate(video_id_list):
                    ...
                    future = executor.submit(self.run_single_video, video_id, gpu_id, neo_hookean_mu_list[i], neo_hookean_damping_list[i], composition_style_list[i])
                ...
                for future in as_completed(future_to_video):
                    ...
                    try:
                        ...
                        logger.info(f"Soft body drop video {video_id} completed on GPU {gpu_id}: {status}")
                    except Exception as e:
                        logger.error(f"Soft body drop video {video_id} failed on GPU {gpu_id} with exception: {e}")
                        with self.lock:
                            self.failed_videos += 1
                    ...
                    logger.info(f"Progress: {completed_count}/{num_videos} soft body drop videos completed "
                               f"({self.generated_videos} success, {self.failed_videos} failed), "
                               f"Rate: {rate:.2f} videos/sec, ETA: {eta/60:.1f} min")
        finally:
            self.cleanup_processes()
        
        logger.info(f"Parallel soft body drop generation completed: {self.generated_videos} successful, "
                   f"{self.failed_videos} failed")

def create_parser():
    """Create argument parser for parallel soft body drop video generation."""
    parser = kb.ArgumentParser()
    
    # Arguments forwarded to run_ball_drop_soft_v4.py
    ...
    parser.add_argument("--num_videos", type=int, default=100,
        help="Total number of soft body drop videos to generate"
    )
...
if __name__ == "__main__":
    ...
    logger.info(f"Generating {args.num_videos} soft body drop videos using {args.num_workers} workers")
    logger.info(f"Each video will vary ball neo-hookean mu and damping with fixed parameters:")
    logger.info(f"Fixed restitution: 0.01 (both ball and platform)")
    logger.info(f"Fixed friction: 0.2 (both ball and platform)")
    logger.info(f"Fixed elastic stiffness: 10.0")
    logger.info(f"Fixed mass: 1.0 kg")
    logger.info(f"Variable neo-hookean mu and damping: 60-600 (high deformability: 60-100, low deformability: 500-600)")
    
    # Run parallel generation
    generator = ParallelSoftBodyDropGenerator(args)
    ...
    try:
        ...
        logger.info("All soft body drop videos generated successfully!")
