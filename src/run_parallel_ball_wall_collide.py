...
class ParallelBallWallCollisionGenerator:
    ...
    def run_single_video(self, video_id: str, gpu_id: int, ball_restitution: float, wall_restitution: float) -> bool:
        """Run a single ball-wall collision video generation using subprocess with specific GPU."""
        try:
            logger.info(f"Generating ball-wall collision video {video_id} on GPU {gpu_id}")
            
            # Build command arguments
            cmd = [
                sys.executable,
                "src/run_ball_wall_collide.py",
                f"--video_id={video_id}",
            ...
            try:
                stdout, stderr = process.communicate(timeout=3000)  # 50 minute timeout (as in ball wall collision script)
                
                if process.returncode == 0:
                    logger.info(f"Successfully generated ball-wall collision video {video_id} on GPU {gpu_id}")
                    with self.lock:
                        self.generated_videos += 1
                    return True
                else:
                    logger.error(f"Failed to generate ball-wall collision video {video_id} on GPU {gpu_id}")
                ...
                process.kill()
                logger.error(f"Ball-wall collision video {video_id} generation timed out on GPU {gpu_id}")
                with self.lock:
                    self.failed_videos += 1
                return False
            finally:
            ...
            logger.error(f"Exception while generating ball-wall collision video {video_id} on GPU {gpu_id}: {e}")
            with self.lock:
                self.failed_videos += 1
            return False
    
    def run_parallel_generation(self, num_workers: int, num_videos: int):
        """Run parallel ball-wall collision video generation using subprocesses with GPU assignment."""
        logger.info(f"Starting parallel ball-wall collision generation: {num_workers} workers, {num_videos} videos")
        ...
        # Sample ball and wall restitution coefficients
        # Create varied combinations for interesting collision dynamics
        ...
        wall_restitution_list = []
        
        for i, video_id in enumerate(video_id_list):
            if restitution_range[i] == 0:
                # Both low restitution (inelastic collisions)
                ball_restitution_list.append(np.random.uniform(0.01, 0.3))
                # wall_restitution_list.append(np.random.uniform(0.01, 0.3))
                wall_restitution_list.append(1.0)
            elif restitution_range[i] == 1:
                # Both high restitution (elastic collisions)
                ball_restitution_list.append(np.random.uniform(0.7, 0.95))
                # wall_restitution_list.append(np.random.uniform(0.7, 0.95))
                wall_restitution_list.append(1.0)
            else:
                # Mixed restitution values for varied dynamics
                ball_restitution_list.append(np.random.uniform(0.01, 0.95))
                # wall_restitution_list.append(np.random.uniform(0.01, 0.95))
                wall_restitution_list.append(1.0)
        ...
        try:
            with ThreadPoolExecutor(max_workers=num_workers) as executor:
                ...
                for i, video_id in enumerate(video_id_list):
                    ...
                    future = executor.submit(self.run_single_video, video_id, gpu_id, ball_restitution_list[i], wall_restitution_list[i])
                ...
                for future in as_completed(future_to_video):
                    ...
                    try:
                        ...
                        logger.info(f"Ball-wall collision video {video_id} completed on GPU {gpu_id}: {status}")
                    except Exception as e:
                        logger.error(f"Ball-wall collision video {video_id} failed on GPU {gpu_id} with exception: {e}")
                        with self.lock:
                            self.failed_videos += 1
                    ...
                    logger.info(f"Progress: {completed_count}/{num_videos} ball-wall collision videos completed "
                               f"({self.generated_videos} success, {self.failed_videos} failed), "
                               f"Rate: {rate:.2f} videos/sec, ETA: {eta/60:.1f} min")
        finally:
            self.cleanup_processes()
        
        logger.info(f"Parallel ball-wall collision generation completed: {self.generated_videos} successful, "
                   f"{self.failed_videos} failed")

def create_parser():
    """Create argument parser for parallel ball-wall collision video generation."""
    parser = kb.ArgumentParser()
    
    # Arguments from run_ball_wall_collide.py
    ...
    parser.add_argument("--wall_friction", type=float, default=0.2)
    ...
    parser.add_argument("--num_videos", type=int, default=100,
        help="Total number of ball-wall collision videos to generate"
    )
...
if __name__ == "__main__":
    ...
    logger.info(f"Generating {args.num_videos} ball-wall collision videos using {args.num_workers} workers")
    logger.info(f"Each video will vary ball and wall restitution coefficients with fixed friction")
    logger.info(f"Fixed friction values - Platform: {args.platform_friction}, Ball: {args.ball_friction}, Wall: {args.wall_friction}")
    
    # Run parallel generation
    generator = ParallelBallWallCollisionGenerator(args)
    ...
    try:
        ...
        logger.info("All ball-wall collision videos generated successfully!")
