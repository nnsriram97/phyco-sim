...
class ParallelBallDropGenerator:
    ...
    def run_single_video(self, video_id: str, gpu_id: int, ball_restitutions: str, platform_restitution: float, composition_style: str, num_balls: int) -> bool:
        """Run a single ball drop video generation using subprocess with specific GPU."""
        try:
            logger.info(f"Generating ball drop video {video_id} with {num_balls} balls on GPU {gpu_id}")
            
            # Build command arguments
            cmd = [
                sys.executable,
                "src/run_ball_drop_v3.py",
                f"--video_id={video_id}",
                ...
                f"--ball_restitutions={ball_restitutions}",
    ...
    def run_parallel_generation(self, num_workers: int, num_videos: int):
        ...
        # Sample number of balls for each video
        # Distribution: varied number of balls (1-5)
        # Use num_balls from args if specified, otherwise sample randomly
        if hasattr(self.base_args, 'num_balls') and self.base_args.num_balls is not None:
            # Use fixed number of balls if specified
            num_balls_list = [self.base_args.num_balls] * num_videos
            logger.info(f"Using fixed number of balls: {self.base_args.num_balls}")
        else:
            # Sample randomly: 10% 2 balls, 40% 3 balls, 30% 4 balls, 20% 5 balls
            num_balls_list = np.random.choice([2, 3], size=num_videos, p=[0.5, 0.5]).tolist()
            logger.info(f"Sampling random number of balls per video (2-5)")
        
        # Sample ball and platform restitution coefficients
        # For each video, sample DIFFERENT restitution for EACH ball
        # Create varied combinations for interesting drop dynamics
        ball_restitutions_list = []  # Will store comma-separated strings like "0.3,0.6,0.9"
        platform_restitution_list = []
        
        # Distribution for restitution sampling strategy per video
        # 30% all low, 30% all high, 40% mixed
        restitution_strategy = np.random.choice([0, 1, 2], size=num_videos, p=[0.15, 0.7, 0.15])
        ...
        for i, video_id in enumerate(video_id_list):
            num_balls = num_balls_list[i]
            
            # Sample individual restitution for each ball
            ball_restitutions_for_video = []
            
            if restitution_strategy[i] == 0:
                # All balls low restitution (inelastic bounces)
                for _ in range(num_balls):
                    sample_ball_restitution = np.random.choice(list(ball_restitution_range.keys()), p=[0.15, 0.7, 0.15])
                    ball_restitutions_for_video.append(np.random.uniform(ball_restitution_range[sample_ball_restitution][0], ball_restitution_range[sample_ball_restitution][1]))
                platform_restitution = np.random.uniform(0.01, 0.3)
                
            elif restitution_strategy[i] == 1:
                # All balls high restitution (elastic bounces)
                for _ in range(num_balls):
                    sample_ball_restitution = np.random.choice(list(ball_restitution_range.keys()), p=[0.15, 0.7, 0.15])
                    ball_restitutions_for_video.append(np.random.uniform(ball_restitution_range[sample_ball_restitution][0], ball_restitution_range[sample_ball_restitution][1]))
                platform_restitution = np.random.uniform(0.7, 0.95)
                
            elif restitution_strategy[i] == 2:
                # Mixed restitution values for varied dynamics
                for _ in range(num_balls):
                    sample_ball_restitution = np.random.choice(list(ball_restitution_range.keys()), p=[0.15, 0.7, 0.15])
                    ball_restitutions_for_video.append(np.random.uniform(ball_restitution_range[sample_ball_restitution][0], ball_restitution_range[sample_ball_restitution][1]))
                platform_restitution = np.random.uniform(0.01, 0.95)
            
            # Convert list of restitutions to comma-separated string
            ball_restitutions_str = ",".join([f"{r:.3f}" for r in ball_restitutions_for_video])
            ball_restitutions_list.append(ball_restitutions_str)
            platform_restitution_list.append(platform_restitution)
        ...
        logger.info(f"Restitution sampling statistics:")
        logger.info(f"  - {np.sum(restitution_strategy == 0)} videos with all low restitution balls")
        logger.info(f"  - {np.sum(restitution_strategy == 1)} videos with all high restitution balls")
        logger.info(f"  - {np.sum(restitution_strategy == 2)} videos with mixed restitution balls")
        logger.info(f"Example ball restitutions: {ball_restitutions_list[0]}")
            ...
            with ThreadPoolExecutor(max_workers=num_workers) as executor:
                ...
                for i, video_id in enumerate(video_id_list):
                    ...
                    future = executor.submit(
                        ...
                        video_id, 
                        gpu_id, 
                        ball_restitutions_list[i],  # Comma-separated string of restitutions
                        platform_restitution_list[i], 
                        composition_style_list[i],
                        num_balls_list[i]
                    )
                    future_to_video[future] = (video_id, gpu_id, num_balls_list[i])
                
                # Monitor progress
                completed_count = 0
                for future in as_completed(future_to_video):
                    completed_count += 1
                    video_id, gpu_id, num_balls = future_to_video[future]
                    
                    try:
                        success = future.result()
                        status = "success" if success else "failed"
                        logger.info(f"Ball drop video {video_id} ({num_balls} balls) completed on GPU {gpu_id}: {status}")
                    except Exception as e:
                        logger.error(f"Ball drop video {video_id} ({num_balls} balls) failed on GPU {gpu_id} with exception: {e}")
                        with self.lock:
                            self.failed_videos += 1
...
def create_parser():
    ...
    # Arguments forwarded to run_ball_drop_v3.py
    ...
    parser.add_argument("--num_balls", type=int, default=None,
                       help="Number of balls to drop (1-5). If not specified, will sample randomly for each video.")
...
if __name__ == "__main__":
    ...
    logger.info(f"Each video will have DIFFERENT restitution coefficients for EACH ball")
    logger.info(f"Fixed friction values - Platform: {args.platform_friction}, Ball: {args.ball_friction}")
    if args.num_balls is not None:
        logger.info(f"Fixed number of balls per video: {args.num_balls}")
    else:
        logger.info(f"Number of balls will be sampled randomly (2-5) for each video")
