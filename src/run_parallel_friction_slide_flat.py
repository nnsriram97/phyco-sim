...
class ParallelFrictionSlideFlatGenerator:
    ...
    def run_single_video(self, video_id: str, gpu_id: int, platform_friction: float, object_friction: float, camera_elevation_angle: float, camera_azimuth_angle: float) -> bool:
        """Run a single friction slide flat video generation using subprocess with specific GPU."""
        try:
            logger.info(f"Generating friction slide flat video {video_id} on GPU {gpu_id}")
            
            # Build command arguments
            cmd = [
                sys.executable,
                "src/run_friction_slide_flat.py",
                f"--video_id={video_id}",
                ...
                f"--prism_friction={platform_friction}",
                ...
                f"--camera_elevation_angle={camera_elevation_angle}",
                f"--camera_azimuth_angle={camera_azimuth_angle}",
            ]
            ...
            if self.base_args.vary_friction_only:
                cmd.append("--vary_friction_only")
            ...
            try:
                ...
                if process.returncode == 0:
                    logger.info(f"Successfully generated friction slide flat video {video_id} on GPU {gpu_id}")
                    with self.lock:
                        self.generated_videos += 1
                    return True
                else:
                    logger.error(f"Failed to generate friction slide flat video {video_id} on GPU {gpu_id}")
                ...
                process.kill()
                logger.error(f"Friction slide flat video {video_id} generation timed out on GPU {gpu_id}")
                with self.lock:
                    self.failed_videos += 1
                return False
            finally:
            ...
            logger.error(f"Exception while generating friction slide flat video {video_id} on GPU {gpu_id}: {e}")
            with self.lock:
                self.failed_videos += 1
            return False
    
    def run_parallel_generation(self, num_workers: int, num_videos: int):
        """Run parallel friction slide flat video generation using subprocesses with GPU assignment."""
        logger.info(f"Starting parallel friction slide flat generation: {num_workers} workers, {num_videos} videos")
        ...
        # Sample platform friction and object friction
        # sample friction range
        friction_range = np.random.choice([0, 1, 2], size=num_videos)
        camera_elevation_angle_list = np.random.uniform(25, 70, size=num_videos)
        camera_azimuth_angle_list = np.random.uniform(0, 360, size=num_videos)
        prism_friction_list = []  # Keep name for compatibility, but represents platform friction
        object_friction_list = []
        for i, video_id in enumerate(video_id_list):
            if friction_range[i] == 0:
                # prism_friction_list.append(np.random.uniform(0.2, 0.5))  # Low platform friction
                # object_friction_list.append(np.random.uniform(0.2, 0.5))  # Low object friction
                prism_friction_list.append(np.random.uniform(0.2, 0.3))  # Low platform friction
                object_friction_list.append(np.random.uniform(0.2, 0.3))  # Low object friction
            elif friction_range[i] == 1:
                prism_friction_list.append(np.random.uniform(0.8, 0.99))  # High platform friction
                object_friction_list.append(np.random.uniform(0.8, 0.99))  # High object friction
            elif friction_range[i] == 2:
                prism_friction_list.append(np.random.uniform(0.2, 0.99))  # Random platform friction
                object_friction_list.append(np.random.uniform(0.2, 0.99))  # Random object friction
            # elif friction_range[i] == 3:
                # prism_friction_list.append(np.random.uniform(0.5, 0.6))  # Random platform friction
                # object_friction_list.append(np.random.uniform(0.5, 0.6))  # Random object friction
        ...
        try:
            with ThreadPoolExecutor(max_workers=num_workers) as executor:
                ...
                for i, video_id in enumerate(video_id_list):
                    ...
                    future = executor.submit(self.run_single_video, video_id, gpu_id, prism_friction_list[i], object_friction_list[i], camera_elevation_angle_list[i], camera_azimuth_angle_list[i])
                ...
                for future in as_completed(future_to_video):
                    ...
                    try:
                        ...
                        logger.info(f"Friction slide flat video {video_id} completed on GPU {gpu_id}: {status}")
                    except Exception as e:
                        logger.error(f"Friction slide flat video {video_id} failed on GPU {gpu_id} with exception: {e}")
                        with self.lock:
                            self.failed_videos += 1
                    ...
                    logger.info(f"Progress: {completed_count}/{num_videos} friction slide flat videos completed "
                               f"({self.generated_videos} success, {self.failed_videos} failed), "
                               f"Rate: {rate:.2f} videos/sec, ETA: {eta/60:.1f} min")
        finally:
            self.cleanup_processes()
        
        logger.info(f"Parallel friction slide flat generation completed: {self.generated_videos} successful, "
                   f"{self.failed_videos} failed")

def create_parser():
    """Create argument parser for parallel friction slide flat video generation."""
    parser = kb.ArgumentParser()
    
    # Arguments from run_friction_slide_flat.py
    ...
    parser.add_argument("--num_videos", type=int, default=100,
        help="Total number of friction slide videos to generate"
    )
...
if __name__ == "__main__":
    ...
    logger.info(f"Generating {args.num_videos} friction slide flat videos using {args.num_workers} workers")
    logger.info(f"Each video will vary friction coefficients on flat platform slides")
    
    # Run parallel generation
    generator = ParallelFrictionSlideFlatGenerator(args)
    ...
    try:
        ...
        logger.info("All friction slide flat videos generated successfully!")
