...
parser.add_argument("--ball_restitution", type=float, default=None,
                   help="Single restitution value for all balls (if ball_restitutions not specified)")
parser.add_argument("--ball_restitutions", type=str, default=None,
                   help="Comma-separated restitution values for each ball (e.g., '0.5,0.7,0.9')")
...
parser.add_argument("--num_balls", type=int, default=3,
                   help="Number of balls to drop (1-5)")
...
class BallDropSimulation:
    ...
    def _configure_kubric(self):
        ...
        renderer = Blender(
            ...
            adaptive_sampling=True,  # Enable adaptive sampling to reduce CPU overhead
    ...
    def _create_drop_balls(self, platform, num_balls=3):
        """Create multiple ball objects that will drop from different heights/positions onto the platform."""
        
        balls = []
        placed_positions = []  # Track XY positions of placed balls
        min_separation = 0.5  # Minimum distance between balls (in meters)
        max_attempts = 100  # Maximum attempts to place each ball
        
        # Get base ball position from URDF (this is the origin offset)
        urdf_path = f"objs/ball_drop/{BALL_NAME}.urdf"
        urdf_origin_position = self._read_urdf_origin_offset(urdf_path)
        if urdf_origin_position is None:
            # Fallback if URDF reading fails
            urdf_origin_position = (0.0, 0.0, 1.0)
        urdf_origin_position = np.array(urdf_origin_position, dtype=float)
        
        base_z = urdf_origin_position[2]  # All balls at same height
        
        # Parse ball restitution values
        ball_restitutions = None
        if self.args.ball_restitutions is not None:
            # Parse comma-separated restitution values
            try:
                ball_restitutions = [float(x.strip()) for x in self.args.ball_restitutions.split(',')]
                logging.info(f"Using specified restitution values: {ball_restitutions}")
                
                # Validate restitution values
                for i, val in enumerate(ball_restitutions):
                    if not (0.0 <= val <= 1.0):
                        logging.warning(f"Restitution value {val} for ball {i} is outside [0, 1] range, clamping")
                        ball_restitutions[i] = max(0.0, min(1.0, val))
                
                # Check if we have enough values
                if len(ball_restitutions) < num_balls:
                    logging.warning(f"Only {len(ball_restitutions)} restitution values provided for {num_balls} balls. "
                                  f"Will sample randomly for remaining balls.")
                elif len(ball_restitutions) > num_balls:
                    logging.warning(f"{len(ball_restitutions)} restitution values provided but only {num_balls} balls. "
                                  f"Using first {num_balls} values.")
                    ball_restitutions = ball_restitutions[:num_balls]
                    
            except ValueError as e:
                logging.error(f"Failed to parse ball_restitutions '{self.args.ball_restitutions}': {e}")
                logging.error("Expected format: '0.5,0.7,0.9' - will use default behavior")
                ball_restitutions = None
        ...
        for i in range(num_balls):
            ...
            # Determine restitution coefficient for this ball
            # Priority: ball_restitutions (per-ball) > ball_restitution (single) > random
            if ball_restitutions is not None and i < len(ball_restitutions):
                # Use specified restitution for this ball
                ball_restitution = ball_restitutions[i]
            elif self.args.ball_restitution is not None:
                # Use single restitution value for all balls
                ball_restitution = self.args.ball_restitution
            else:
                # Random restitution
                ball_restitution = self.rng.uniform(0.1, 0.9)
            
            # First ball stays at default position, others placed with minimum separation
            if i == 0:
                # Ball 0: Keep at default URDF origin position (no offset)
                x_offset = 0.0
                y_offset = 0.0
            else:
                # Other balls: Place with minimum separation from all existing balls
                # Use rejection sampling to ensure proper spacing
                placed = False
                for attempt in range(max_attempts):
                    # Random position in a reasonable area around default position
                    # Increase search radius to allow more spacing options
                    x_offset = self.rng.uniform(-0.8, 0.8)
                    y_offset = self.rng.uniform(-0.8, 0.8)
                    
                    candidate_xy = np.array([
                        urdf_origin_position[0] + x_offset,
                        urdf_origin_position[1] + y_offset
                    ])
                    
                    # Check distance from all previously placed balls
                    valid_position = True
                    for prev_xy in placed_positions:
                        distance = np.linalg.norm(candidate_xy - prev_xy)
                ...
                if not placed:
                    # If we couldn't find a valid position, use a fallback strategy
                    # Place ball at a position that maximizes distance from existing balls
                    logging.warning(f"Could not find well-separated position for ball {i} after {max_attempts} attempts")
                    # Use a radial placement strategy as fallback
                    angle = (2 * np.pi * i) / num_balls  # Evenly distribute around circle
                    radius = min_separation * 1.2  # Slightly larger than minimum
                    x_offset = radius * np.cos(angle)
                    y_offset = radius * np.sin(angle)
            
            # All balls at same Z height (no height variation)
            z_offset = 0.0
            
            # Calculate desired position for this ball (where we want it to be)
            ...
            # Track this ball's XY position
            placed_positions.append(desired_position[:2].copy())
            ...
            ball = kb.FileBasedObject(
                ...
                scale=1.0,
                position=tuple(desired_position),
                mass=ball_mass,
                friction=ball_friction,
                restitution=ball_restitution,
                segmentation_id=self._get_segmentation_id(),
                urdf_origin_offset=tuple(urdf_origin_position),
            )
            ...
            # Adjust Blender object location to account for URDF origin offset
            # This follows the same pattern as pool table code
            if hasattr(ball, "urdf_origin_offset"):
                blender_obj = ball.linked_objects[self.renderer]
                offset = np.array(ball.urdf_origin_offset)
                blender_obj.location = desired_position - offset
            ...
            ball._ball_index = i
            ball._urdf_origin_position = urdf_origin_position.tolist()
            ball._desired_position = desired_position.tolist()
            ball._position_offset = [x_offset, y_offset, z_offset]
            
            balls.append(ball)
            
            if i == 0:
                logging.info(f"Created ball_{i} ({BALL_NAME}) at DEFAULT position {desired_position.tolist()} with restitution {ball_restitution:.3f}")
            else:
                # Calculate separation from ball 0 for logging
                separation_from_default = np.linalg.norm(desired_position[:2] - placed_positions[0])
                logging.info(f"Created ball_{i} ({BALL_NAME}) at position {desired_position.tolist()} (xy_offset: x={x_offset:.3f}, y={y_offset:.3f}, separation={separation_from_default:.3f}m) with restitution {ball_restitution:.3f}")
            logging.info(f"  URDF origin offset: {urdf_origin_position.tolist()}")
        
        logging.info(f"Created {num_balls} balls that will drop naturally under gravity")
        
        # Verify and log minimum separation between all balls
        if len(placed_positions) > 1:
            min_actual_separation = float('inf')
            for i in range(len(placed_positions)):
                for j in range(i + 1, len(placed_positions)):
                    dist = np.linalg.norm(placed_positions[i] - placed_positions[j])
                    min_actual_separation = min(min_actual_separation, dist)
            logging.info(f"✓ Minimum separation between balls: {min_actual_separation:.3f}m (required: {min_separation:.3f}m)")
            if min_actual_separation < min_separation * 0.95:  # Allow 5% tolerance
                logging.warning(f"⚠️  Some balls may be closer than required minimum separation!")
        
        # Store ball names in metadata
        self.metadata["ball_name"] = BALL_NAME
        self.metadata["num_balls"] = num_balls
        self.metadata["ball_urdf_origin_position"] = urdf_origin_position.tolist()
        self.metadata["min_ball_separation"] = min_separation
    ...
    def _collect_all_object_metadata(self):
        ...
        # Add drop balls (support both single ball and multiple balls)
        if hasattr(self, 'balls') and self.balls:
            for i, ball in enumerate(self.balls):
                all_objects.append((f'ball_{i}', ball))
        elif hasattr(self, 'ball') and self.ball:
    ...
    def _setup_camera_with_blender_align(self, objects):
        ...
        # Get all ball positions (support both single and multiple balls)
        ...
        if hasattr(self, 'balls') and self.balls:
            ...
            for ball in self.balls:
                ball_initial_positions.append(np.array(ball.position))
                
            if hasattr(self, 'cube_platform') and self.cube_platform:
                platform_position = np.array(self.cube_platform.position)
                for _ in self.balls:
                    ball_final = platform_position.copy()
                    ball_final[2] -= 0.2  # Add some height for ball radius
                    ball_final_positions.append(ball_final)
        elif hasattr(self, 'ball') and self.ball:
            # Single ball (backward compatibility)
            ball_initial_positions.append(np.array(self.ball.position))
            ...
            # Fallback defaults
            ball_initial_positions.append(np.array([0.0, 0.0, 1.0]))
            ball_final_positions.append(np.array([0.0, 0.0, 0.0]))

        # Calculate bounding box for all balls
        all_positions = ball_initial_positions + ball_final_positions
        min_bound = np.min(all_positions, axis=0)
        max_bound = np.max(all_positions, axis=0)
        
        # Calculate trajectory parameters based on all balls
        trajectory_start = np.mean(ball_initial_positions, axis=0)  # Center of all initial positions
        trajectory_end = np.mean(ball_final_positions, axis=0)      # Center of all final positions
        trajectory_center = (trajectory_start + trajectory_end) / 2.0
        
        # Calculate the maximum extent for proper framing
        trajectory_height = abs(max_bound[2] - min_bound[2])
        trajectory_width = max(abs(max_bound[0] - min_bound[0]), abs(max_bound[1] - min_bound[1]))
        ...
        # We want all balls' initial positions at the top of the frame and final positions visible
        ...
        # Also consider horizontal extent for multiple balls
        horizontal_extent_needed = trajectory_width * 1.2  # 20% margin
        ...
        # Calculate distance needed for horizontal framing (for multiple balls)
        distance_for_horizontal = horizontal_extent_needed / (2 * np.tan(horizontal_fov / 2))
        
        # Use the larger of the two distances to ensure everything fits
        distance_for_framing = max(distance_for_vertical, distance_for_horizontal)
        
        # Apply distance variation for diversity
        camera_distance = distance_for_framing * 1.1 * distance_variation_factor
        ...
        num_balls = len(ball_initial_positions)
        logging.info(f"🎥 CAMERA SETUP FOR {num_balls} BALL DROP TRAJECTORY WITH DIVERSITY")
        ...
        logging.info(f"Ball initial positions: {[list(pos) for pos in ball_initial_positions]}")
        logging.info(f"Ball final positions: {[list(pos) for pos in ball_final_positions]}")
        ...
        logging.info(f"Trajectory width: {trajectory_width:.2f}")
        ...
        self._verify_trajectory_framing(ball_initial_positions, ball_final_positions)
        
        # Apply depth of field to focus on the balls and blur background
        # Use the center of all initial positions as the focus point
        if elevation_angle < np.radians(15):
            center_ball_position = np.mean(ball_initial_positions, axis=0)
            self._setup_camera_depth_of_field(center_ball_position, camera_position, look_at_point)
        
        # Log camera diversity verification
        self._log_camera_diversity_verification(trajectory_start, trajectory_end, look_at_point)
    ...
    def _verify_trajectory_framing(self, ball_initial_positions, ball_final_positions):
        """Verify that the ball trajectories are properly framed in the camera view."""
        ...
        num_balls = len(ball_initial_positions)
        logging.info(f"📋 TRAJECTORY FRAMING VERIFICATION FOR {num_balls} BALLS:")
        
        all_init_coords = []
        all_final_coords = []
        
        # Check each ball's trajectory
        for i, (init_pos, final_pos) in enumerate(zip(ball_initial_positions, ball_final_positions)):
            init_x, init_y = project_to_camera_plane(init_pos)
            final_x, final_y = project_to_camera_plane(final_pos)
            
            logging.info(f"Ball {i}:")
            
            if init_x is not None and init_y is not None:
                logging.info(f"  Initial position screen coords: ({init_x:.2f}, {init_y:.2f})")
                all_init_coords.append((init_x, init_y))
                if abs(init_x) <= 1.0 and abs(init_y) <= 1.0:
                    logging.info(f"  ✅ Initial position is IN FRAME")
                else:
                    logging.warning(f"  ❌ Initial position is OUT OF FRAME")
            else:
                logging.warning(f"  ❌ Initial position is BEHIND CAMERA")
                
            if final_x is not None and final_y is not None:
                logging.info(f"  Final position screen coords: ({final_x:.2f}, {final_y:.2f})")
                all_final_coords.append((final_x, final_y))
                if abs(final_x) <= 1.0 and abs(final_y) <= 1.0:
                    logging.info(f"  ✅ Final position is IN FRAME")
                else:
                    logging.warning(f"  ❌ Final position is OUT OF FRAME")
            else:
                logging.warning(f"  ❌ Final position is BEHIND CAMERA")
        
        # Calculate overall trajectory coverage
        if all_init_coords and all_final_coords:
            all_coords = all_init_coords + all_final_coords
            ...
            logging.info(f"📏 Overall trajectory span: vertical={vertical_span:.2f}, horizontal={horizontal_span:.2f}")
    ...
    def run(self):
        ...
        # Phase 2: Add drop balls
        # Debug pause after scene setup if GUI is enabled
        if self.args.debug_gui:
            self.debug_pause_at_key_points()
        
        self.balls = self._create_drop_balls(self.cube_platform, num_balls=self.args.num_balls)
        
        # Collect all objects for camera and metadata
        all_objects = [self.cube_platform] + self.balls
        ...
        self.metadata["simulation_type"] = "ball_drop_multiple"
        ...
        # Store metadata for each ball
        self.metadata["balls_info"] = []
        ball_xy_positions = []
        for i, ball in enumerate(self.balls):
            ball_position = getattr(ball, '_desired_position', list(ball.position))
            ball_xy_positions.append(ball_position[:2])
            
            ball_info = {
                "index": i,
                "name": ball.name,
                "friction": getattr(ball, '_friction_coefficient', 'unknown'),
                "restitution": getattr(ball, '_restitution', 'unknown'),
                "mass": getattr(ball, 'mass', 1.0),
                "initial_position": ball_position,
                "urdf_origin_position": getattr(ball, '_urdf_origin_position', None),
                "position_offset": getattr(ball, '_position_offset', [0.0, 0.0, 0.0]),
                "is_default_position": (i == 0),
            }
            self.metadata["balls_info"].append(ball_info)
        
        # Calculate and store pairwise distances between balls
        if len(self.balls) > 1:
            pairwise_distances = []
            for i in range(len(ball_xy_positions)):
                for j in range(i + 1, len(ball_xy_positions)):
                    dist = float(np.linalg.norm(np.array(ball_xy_positions[i]) - np.array(ball_xy_positions[j])))
                    pairwise_distances.append({
                        "ball_i": i,
                        "ball_j": j,
                        "distance": dist
                    })
            self.metadata["ball_pairwise_distances"] = pairwise_distances
            self.metadata["min_ball_separation_actual"] = min([d["distance"] for d in pairwise_distances])
        ...
        logging.info("Phase 3: Starting multiple ball drop simulation...")
        logging.info(f"Number of balls: {len(self.balls)}")
        ...
        for i, ball in enumerate(self.balls):
            logging.info(f"Ball {i} ({BALL_NAME}): friction={getattr(ball, '_friction_coefficient', 'unknown'):.3f}, restitution={getattr(ball, '_restitution', 'unknown'):.3f}, position={list(ball.position)}")
        ...
        # Check each ball's movement
        for i, ball in enumerate(self.balls):
            if ball in anim_data:
                positions = anim_data[ball]["position"]
                velocities = anim_data[ball]["velocity"]
                ...
                logging.info(f"Ball {i}: start={start_pos}, end={end_pos}, max_vel={max_velocity:.3f}")
            else:
                logging.warning(f"Ball {i} not found in animation data")
